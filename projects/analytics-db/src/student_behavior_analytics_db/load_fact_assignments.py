...
def load_fact_assignments(
    ...
    for row in rows:
        ...
        submitted_at = _normalize_timestamp(
            _first_value(row, "ANSWER_TIME", "CREATER_TIME", "INSERT_TIME")
        )
        term_key = _pick_term_key(row, submitted_at, term_calendar)
        ...
        records.append(
            {
                ...
                "term_key": term_key,
                "work_id": _normalize_number(_first_value(row, "WORK_ID", "work_id")),
                "work_title": _normalize_text(_first_value(row, "WORK_TITLE", "work_title")),
                ...
                "status": _normalize_number(_first_value(row, "STATUS", "status")),
                "fullmarks": _normalize_number(_first_value(row, "FULLMARKS", "fullmarks")),
                "score": _normalize_number(_first_value(row, "SCORE", "score")),
                "type": _normalize_number(_first_value(row, "TYPE", "type")),
                "submitted_at": submitted_at,
                "reviewed_at": _normalize_timestamp(
                    _first_value(row, "REVIEW_TIME", "reviewed_at")
                ),
    ...
    return _as_frame(records, _ASSIGNMENT_COLUMNS)
...
def _pick_term_key(
    row: dict[str, Any], submitted_at: str | None, term_calendar: list[dict[str, Any]]
) -> str | None:
    ...
    return _term_key_from_calendar(submitted_at, term_calendar) or infer_term_from_month_only(submitted_at)
...
def _term_key_from_calendar(submitted_at: str | None, term_calendar: list[dict[str, Any]]) -> str | None:
    if submitted_at is None or not term_calendar:
        return None
    try:
        submitted_date = pd.to_datetime(submitted_at, errors="coerce").date()
    ...
    for term in term_calendar:
        ...
        if pd.isna(start_date) or pd.isna(end_date):
            continue
        if start_date.date() <= submitted_date <= end_date.date():
